...
namespace ORB_SLAM3
{
    ...
    Sophus::SE3f System::TrackMonocularAndFire(const cv::Mat& im, const double& timestamp,
                                               const vector<GPS::NavSatFix>& vGpsMeas,
                                               const vector<cv::Rect>& fire_spots, string filename)
    {
        {
            unique_lock<mutex> lock(mMutexReset);
            if (mbShutDown)
        ...
        {
            cerr <<
                "ERROR: you called TrackMonocularAndFire() but input sensor was not set to Monocular nor Monocular-GPS."
                << endl;
            exit(-1);
        }

        cv::Mat imToFeed = im.clone();
        vector<cv::Rect> resized_fire_spots;
        if (settings_ && settings_->needToResize())
        {
            cv::Mat resizedIm;
            cv::resize(im, resizedIm, settings_->newImSize());
            imToFeed = resizedIm;
            // resize bounding boxes as well
            for (int i = 0; i < fire_spots.size(); i++)
            {
                cv::Rect resized_fire_spot;
                resized_fire_spot.x = fire_spots[i].x * settings_->newImSize().width / im.cols;
                resized_fire_spot.y = fire_spots[i].y * settings_->newImSize().height / im.rows;
                resized_fire_spot.width = fire_spots[i].width * settings_->newImSize().width / im.cols;
        ...
        // Check reset
        {
            ...
            {
                cout << "SYSTEM-> Reseting active map in monocular-GPS case" << endl;
                mpTracker->ResetActiveMap();
                mbResetActiveMap = false;
            }
        }
