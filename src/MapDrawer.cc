    ...
    void MapDrawer::DrawMapPoints()
    {
        ...
        for (set<MapPoint*>::iterator sit = spRefMPs.begin(), send = spRefMPs.end(); sit != send; sit++)
        {
            ...
            if ((*sit)->CheckPointType() == 1)
            {
                ...
                // std::cout << (*sit)->GetWorldPos() << std::endl;
                // // print camera pose
                // std::cout << "======>Camera coordinates:" << std::endl;
                // std::cout << (*sit)->GetReferenceKeyFrame()->GetPoseInverse().translation() << std::endl;
                // // print distance between point and camera
                // std::cout << "======>Distance:" << std::endl;
                // std::cout << (*sit)->GetReferenceKeyFrame()->GetPoseInverse().translation().norm() << std::endl;
                // store fire spot coordinates in Map::mvFireSpots
                mpAtlas->GetCurrentMap()->mvFireSpots.push_back((*sit)->GetWorldPos());
            }
        }
        glEnd();
